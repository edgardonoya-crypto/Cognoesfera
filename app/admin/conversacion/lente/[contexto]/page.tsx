...
export default function LentePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!session || session.user.email !== ARQUITECTO_EMAIL) { setStatus('unauth'); return }
        ...
        <div style={s.header}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' as const }}>
              <span style={{ fontSize: '1rem', fontWeight: 600, color: '#18201e' }}>{contexto}</span>
            ...
            <span style={{ fontSize: '0.78rem', color: '#8a9e98' }}>{convs.length} conversación{convs.length !== 1 ? 'es' : ''} · {new Set(convs.map(c => c.email_participante)).size} usuario{new Set(convs.map(c => c.email_participante)).size !== 1 ? 's' : ''}</span>
          </div>
        </div>

        {convs.length === 0 && <p style={s.empty}>No hay conversaciones en este contexto.</p>}

        {convs.map((conv, idx) => (
          <div key={conv.id}>
            {/* Separador con email + fecha */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, margin: idx === 0 ? '0 0 16px' : '32px 0 16px' }}>
              <div style={{ flex: 1, height: 1, background: 'rgba(34,58,54,.10)' }} />
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, flexShrink: 0 }}>
                <span style={{ fontSize: '0.75rem', color: '#4eaa98', fontWeight: 500 }}>{conv.email_participante ?? '—'}</span>
                <span style={{ fontSize: '0.68rem', color: '#8a9e98' }}>{fmt(conv.created_at)} · {fmtHora(conv.created_at)}</span>
              </div>
              <div style={{ flex: 1, height: 1, background: 'rgba(34,58,54,.10)' }} />
            </div>
